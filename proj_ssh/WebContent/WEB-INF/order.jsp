...
<%
	  	ArrayList<OrderItemInfo> items
	      = (ArrayList<OrderItemInfo>)request.getAttribute("order");
	    for(OrderItemInfo item : items) {
	  %>
<tr>
  <td class="o-id"><%= item.getId().toString() %></td>
  <td class="o-isbn"><%= item.getIsbn() %></td>
  <td class="o-cnt"><%= item.getNum().toString() %></td>
  <td class="o-time"><%
